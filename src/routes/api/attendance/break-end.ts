...
import { breakEndHandler } from '@/features/attendance/server/handlers.server'

export const Route = createFileRoute('/api/attendance/break-end')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const entry = await breakEndHandler(r.ctx)
