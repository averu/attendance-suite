...
import { clockOutHandler } from '@/features/attendance/server/handlers.server'

export const Route = createFileRoute('/api/attendance/clock-out')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const entry = await clockOutHandler(r.ctx)
