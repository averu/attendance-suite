...
import { approveLeaveRequestHandler } from '@/features/leave-request/server/handlers.server'
import { ReviewLeaveRequestInputSchema } from '@/features/leave-request/schemas'

export const Route = createFileRoute('/api/leave-requests/approve')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const result = await approveLeaveRequestHandler(r.ctx, parsed.data)
